  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        {[
          { label: 'Total Revenue', value: stats.total, color: 'from-[#38BDF8] to-[#0EA5E9]' },
                ...
                {payments.map((p) => (
                  <tr key={p.id} className="border-b border-gray-50 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="p-4 text-right">
                      {p.status !== 'paid' && (
                        <button onClick={() => handleMarkPaid(p.id)} className="px-3 py-1.5 rounded-lg bg-[#38BDF8] text-white hover:bg-[#0EA5E9] transition-colors text-xs font-medium">Mark Paid</button>
                      )}
                    </td>
                  </tr>
                ))}
